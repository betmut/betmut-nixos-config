...
- **Time-aware wallpaper changer**: a small script that updates your wallpaper based on time of day (located at `modules/services/scripts/change-wallpaper.sh`); default keybinding `SUPER+SHIFT+W` (fully customizable) to change your wallpaper and `SUPER+W` to open `waypaper` (hyprland only).
